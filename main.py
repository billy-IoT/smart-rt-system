...
# 2. DATABASE & MEMORY
kas_rt = {"total": 5000000}
warga_database = {} # Format: {uid: {'name': '...', 'username': '...'}}
user_states = {} 
pending_approvals = {}
chat_history = {} 
...
def main_handler(message):
    ...
    # WARRANTY SYSTEM (Japri warga yang di-tag)
    ...
    for username in mentioned:
        ...
        if target_uid:
            try: bot.send_message(target_uid, f"⚠️ *Warning/Warranty*:\nAnda di-tag oleh {warga_database[uid]['name']} dalam laporan: '{text}'")
            except: pass
    ...
    if is_admin(uid) or f"@{bot.get_me().username}" in text or message.reply_to_message:
        ...
        prompt = "Anda asisten RT Smart. Jawab gaul, sopan, solutif, jangan halu. Kas RT: " + str(kas_rt['total'])
...
bot.infinity_polling()    # State Machine Iuran
    if uid in user_states:
        handle_lapor_steps(message)
        return
